...
class uploadpicture(Resource):
    def get(self, orderid, date, count):
        ...
        res = md.selectdb("select * from orders as o join photo as p on o.orderid = p.orderid where o.orderid = '%s'"%(orderid))
...
class search(Resource):
    def get(self, columnName, data):
        ...
        md = MufiData()
        res = md.selectdb("select o.ordername, o.orderid, o.pinnumber, u.name, o.state, EXISTS (select * from photo where orderid = o.orderid) as success  from orders as o join user as u on o.userid = u.userid where %s"%(columnName) + " like '%"+ data +"%'")
